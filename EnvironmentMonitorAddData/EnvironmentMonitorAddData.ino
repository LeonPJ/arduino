...
void COState() {
  ...
  while((millis() - ThirdPage) < OLEDSreemHold) {// 一氧化碳
    ...
    String SSCO = SCO + String(" ppm");
    //Serial.println(CO);
    display.clear();
    display.drawString(OLEDItemX, OLEDItemY, "CO:");
    display.drawString(OLEDDataX, OLEDDataY, SSCO);
    display.display();
    }
  return SCO;
}
...
void CO2State() {
  ...
  while((millis() - FourthPage) < OLEDSreemHold) {// 二氧化碳
    ...
    String SSCO2 = CO2 + String(" ppm");
    //Serial.println(SCO2);
    display.clear();
    display.drawString(OLEDItemX, OLEDItemY, "CO2:");
    display.drawString(OLEDDataX, OLEDDataY, SSCO2);
    display.display();
  }
  retrun SCO2;
}
...
void LPGState() {
  ...
  while((millis() - FifthPage) < OLEDSreemHold) {// 液化石油氣
    ...
    String SSLPG = SLPG + String(" ppm");
    Serial.println(SLPG);
    display.clear();
    display.drawString(OLEDItemX, OLEDItemY, "LPG:");
    display.drawString(OLEDDataX, OLEDDataY, SSLPG);
    display.display();
    }
  retrun SLPG;
}
